...
const getTimeLine = async () => {
  ...
  const dataCitySum = (title) => {
    ...
    const suma = (dataCity, key, type) =>
      dataSUM(dataCity.map((i) => i[key][type]));

    return Object.keys(data).map((keyF) =>
      keys.map(
        (d) =>
          data[keyF].map(() => ({
  ...
  const servicesss = ['Colombia', 'Venezuela'];
  const dataNoCity = datosRecive
    .map((i) =>
      i.filter(
        (country) =>
          !mapFilterCountry.includes(country.Country) &&
          !servicesss.includes(country.Country)
      )
    )
    .filter((val) => val.length > 1);
  const cityJsonInfo = () => {
